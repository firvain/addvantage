...
app.get('/getAdvantagePrimaryData', async (req, res) => {
  try {
    ...
    sessionId = loginResult.response?.result?.[0]?.['string']?.[0];
    ...
    for (const ws of weatherStationsList) {
      ...
      mapNodesToProperty(['98', '144'], 'Precipitation');
    ...
    // Logout from the Advantage API
    await client.get(`/addUPI?function=logout&session-id=${sessionId}`);
  }
});
